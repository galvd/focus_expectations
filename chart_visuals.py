import pandas as pd
import os
import matplotlib.pyplot as plt
import matplotlib.dates as mdates
from matplotlib.lines import Line2D  # legenda customizada
from settings.settings import load_config



config = load_config()



def carregar_dados(proj_dir, arquivo_focus, arquivo_sgs):
    df_focus = pd.read_csv(os.path.join(proj_dir, arquivo_focus))
    
    # Adicionado format='mixed' para evitar falhas de inferência de data
    df_focus['data_boletim'] = pd.to_datetime(df_focus['data_boletim'], format='mixed')
    df_focus['data_alvo'] = pd.to_datetime(df_focus['periodo_previsao'].astype(str) + '-12-31')
    
    df_sgs = pd.read_csv(os.path.join(proj_dir, arquivo_sgs))
    df_sgs['data'] = pd.to_datetime(df_sgs['data'], format='mixed')
    
    return df_focus, df_sgs

def plotar_trajetorias(indicador, df_focus, df_sgs, data_inicio):
    df_real = df_sgs[(df_sgs['indicador'] == indicador) & (df_sgs['data'] >= data_inicio)].copy()
    if df_real.empty:
        return

    df_ind = df_focus[(df_focus['indicador'] == indicador) & (df_focus['data_boletim'] >= data_inicio)].copy()
    if df_ind.empty:
        return

    boletins_selecionados = df_ind['data_boletim'].unique()

    plt.figure(figsize=(14, 7))
    plt.style.use('default')

    # Plota a linha Real
    plt.plot(df_real['data'], df_real['valor'], color='#2979FF', linewidth=2.5, zorder=5)

    for dt_bol in boletins_selecionados:
        df_bol = df_ind[df_ind['data_boletim'] == dt_bol].sort_values('data_alvo')
        
        if df_bol.empty:
            continue

        real_historico = df_real[df_real['data'] <= dt_bol]
        if not real_historico.empty:
            prev_x = dt_bol
            prev_y = real_historico.iloc[-1]['valor']
        else:
            prev_x = None
            prev_y = None

        for _, row in df_bol.iterrows():
            curr_x = row['data_alvo']
            curr_y = row['previsao']
            
            ano_bol = pd.to_datetime(dt_bol).year
            horizonte = curr_x.year - ano_bol

            # Cores e opacidades
            if horizonte <= 0:
                cor, alfa = '#424242', 0.20  # Cinza escuro
            elif horizonte == 1:
                cor, alfa = '#9E9E9E', 0.10  # Cinza médio
            else:
                cor, alfa = '#E0E0E0', 0.05  # Cinza claro

            if prev_x is not None:
                plt.plot([prev_x, curr_x], [prev_y, curr_y], color=cor, alpha=alfa, linewidth=1.0, zorder=2)

            prev_x = curr_x
            prev_y = curr_y

    indicador_formatado = indicador.replace('$', r'\$')
    plt.title(f'Trajetória de Expectativas Focus vs Realizado: {indicador_formatado}', fontsize=14, fontweight='bold', pad=15)
    plt.ylabel('Percentual (%)' if '%' in indicador_formatado else 'Valor', fontsize=11)
    
    plt.gca().xaxis.set_major_locator(mdates.YearLocator())
    plt.gca().xaxis.set_major_formatter(mdates.DateFormatter('%Y'))
    plt.xticks(rotation=45)
    
    plt.gca().spines['top'].set_visible(False)
    plt.gca().spines['right'].set_visible(False)

    # --- LEGENDA CUSTOMIZADA (Opacidade 100%) ---
    elementos_legenda = [
        Line2D([0], [0], color='#2979FF', lw=2.5, label='Dado Real'),
        Line2D([0], [0], color='#424242', lw=1.5, label='Expectativa (Ano Corrente)'),
        Line2D([0], [0], color='#9E9E9E', lw=1.5, label='Expectativa (1 Ano à Frente)'),
        Line2D([0], [0], color='#E0E0E0', lw=1.5, label='Expectativa (2+ Anos à Frente)')
    ]
    plt.legend(handles=elementos_legenda, frameon=False, loc='upper left')
    # --------------------------------------------

    plt.tight_layout()
    
    
    pasta_charts = os.path.join(os.getcwd(), 'charts')
    os.makedirs(pasta_charts, exist_ok=True)
    
    nome_arquivo = f"trajetoria_{indicador.replace(' ', '_').replace('%', 'pct').replace('/', '')}.png"
    caminho_salvar = os.path.join(pasta_charts, nome_arquivo)
    
    plt.savefig(caminho_salvar, dpi=300)
    plt.close()
    print(f"Gráfico salvo: {caminho_salvar}")

if __name__ == "__main__":
    df_f, df_s = carregar_dados(proj_dir=os.getcwd())
    
    indicadores = df_s['indicador'].unique()
    for ind in indicadores:
        plotar_trajetorias(ind, df_f, df_s, data_inicio='2010-01-01')